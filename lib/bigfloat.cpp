...
namespace longarithmetic {
    namespace {
        BigFloat _root(const BigFloat &num, const uint32_t &deg) {
            ...
            BigFloat Xk =
                    (10_bf).power((std::max(A.size(), A.precision()) - A.precision() + deg - 1) / deg) + precision;
            BigFloat prev = 1_bf;
        ...
        BigFloat _power(const BigFloat &num, const uint32_t &deg) {
            if (deg == 1) {
                return num;
            } else if (deg % 2 == 0) {
                BigFloat tmp = _power(num, deg / 2);
                return tmp * tmp;
            }
            return num * _power(num, deg - 1);
        }
    }
    ...
    BigFloat BigFloat::power(const uint32_t &deg) const {
        ...
        BigFloat temp = _power(this->shift(static_cast<int32_t>(_pre)).as_integer(), deg);
...
longarithmetic::BigFloat operator ""_bf(long double data) {
    return longarithmetic::BigFloat(data);
}

longarithmetic::BigFloat operator ""_bf(unsigned long long data) {
    return longarithmetic::BigFloat(data);
}